...
- [x] Haushalts-Verwaltung: Haushalt anlegen, Einladungslink generieren/anzeigen, Einladung annehmen — inkl. Haushalts-Switcher im Header; kein Backend-Endpunkt für Mitgliederliste/-entfernung vorhanden, daher keine entsprechende UI. **Follow-up bei Umsetzung:** sobald ein Mitglied-entfernen/Haushalt-verlassen-Endpunkt existiert, muss er nach dem `Membership`-Delete `RealtimeService.evictFromHousehold(userId, householdId)` (`apps/backend/src/realtime/realtime.service.ts`, seit Phase 3 Slice 1 vorhanden) aufrufen, damit bereits verbundene WebSocket-Sockets sofort aus dem Haushalts-Raum entfernt werden statt bis zum nächsten Verbindungsabbruch weiter Broadcasts zu empfangen
- [x] Kind-Profil-Verwaltung UI (Anlegen/Bearbeiten/Löschen) — inkl. Foto-Upload mit Client-Vorprüfung; „Foto entfernen" ist noch nicht möglich, da das Backend das nicht unterstützt
