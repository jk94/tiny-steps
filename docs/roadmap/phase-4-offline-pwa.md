...
### Tests
- [x] Test: Eingabe ohne Netzwerkverbindung möglich, Daten gehen nicht verloren — `offlineNoDataLoss.integration.spec.ts`: erfasst offline (nur `apiFetch` gemockt, echtes fake-indexeddb) alle drei Domains und alle Operationstypen (Feeding-/Diaper-Create, Feeding-Update, Feeding-Timer-Stop), prüft die vier gepufferten Records direkt gegen IndexedDB und beweist mit einem simulierten Reload (nur `vi.resetModules()`, gleiche `IDBFactory`) die Persistenz über den echten IndexedDB-Speicher statt über In-Memory-Modulzustand
