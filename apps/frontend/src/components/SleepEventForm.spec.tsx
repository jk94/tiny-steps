...
describe('SleepEventForm (create mode)', () => {
  it('renders the occurred-at, start-time, and end-time fields', () => {
  ...
  it('blocks submission and shows a validation error when endedAt is before startedAt', async () => {
    const onSubmit = vi.fn();
    const user = userEvent.setup();
    renderForm('create', onSubmit);
  ...
  it('submits a fully-backfilled entry with the expected shape', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({
      occurredAt: new Date('2026-01-01T20:00').toISOString(),
      startedAt: new Date('2026-01-01T20:00').toISOString(),
...
describe('SleepEventForm (edit mode)', () => {
  ...
  it('allows an empty endedAt (an in-progress timer stays editable)', async () => {
    ...
    // Round-tripping the pre-filled ISO timestamps through the
    // datetime-local input and back must reproduce the original values —
