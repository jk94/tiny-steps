 ...
 * unchanged) value. `SleepService.update` re-checks the merged start/end
 * pair itself after loading the existing row; see its doc comment.
 *
 * This is a deliberate copy of
 * `apps/backend/src/feeding/validators/is-end-not-before-start.validator.ts`,
 * scoped to `sleep` — not yet consolidated, since Sleep is only the second
 * consumer (see that copy's own doc comment for the "third consumer"
 * consolidation threshold this project follows, already applied to
 * `IsNotFutureDate`).
 */
