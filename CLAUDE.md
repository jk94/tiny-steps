...
- **Auth**: local email/password (Argon2 hashing) *and* OIDC (Authorization Code Flow + PKCE) coexist; users pick a method at login. One or more OIDC providers (Keycloak, Authentik, Google, Entra ID, …) are registered via config file, not code changes.
- **Data model** (see PRD 5.2): `User —< Membership >— Household —< Child —< Event`. Every `Event` (Feeding/Sleep/Diaper/…) references both the `Child` and the `User` who logged it. Authorization is checked via the user's `Membership` role in the `Household`.
